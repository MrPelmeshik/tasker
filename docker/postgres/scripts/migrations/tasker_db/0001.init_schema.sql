...
create table if not exists events_2_task ( 
    ...
    deactivated_at timestamptz,
    is_active boolean not null default true,
    constraint unique_active_event_task unique (event_id, task_id) where (is_active = true)
);
...
create table if not exists events_2_subtask (
    ...
    deactivated_at timestamptz,
    is_active boolean not null default true,
    constraint unique_active_event_subtask unique (event_id, subtask_id) where (is_active = true)
);
...
create table if not exists events_2_group (
    ...
    deactivated_at timestamptz,
    is_active boolean not null default true,
    constraint unique_active_event_group unique (event_id, group_id) where (is_active = true)
);
...
create table if not exists events_2_area (
    ...
    deactivated_at timestamptz,
    is_active boolean not null default true,
    constraint unique_active_event_area unique (event_id, area_id) where (is_active = true)
);
...
create table if not exists events_2_purpose (
    ...
    deactivated_at timestamptz,
    is_active boolean not null default true,
    constraint unique_active_event_purpose unique (event_id, purpose_id) where (is_active = true)
);
